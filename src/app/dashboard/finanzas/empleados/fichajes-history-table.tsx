...
const fichajeTypes = ['Entrada', 'Salida', 'Inicio Descanso', 'Fin Descanso'];
...
export function FichajesHistoryTable({ allFichajes, employees }: FichajesHistoryTableProps) {
  ...
  const filteredFichajes = useMemo(() => {
    ...
    return allFichajes.filter(f => {
        ...
        const byType = filtroTipo === 'all' || f.type === filtroTipo;
        return isInRange && byEmployee && byType;
    });

  }, [allFichajes, period, customDateRange, filtroEmpleado, filtroTipo]);
  ...
  const handleExport = () => {
    ...
    const rows = filteredFichajes.map(f => {
        ...
        const employeeName = employees.find(e => e.id === f.employeeId)?.name || f.employeeId;
        const type = f.type;
  ...
  return (
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>Consulta y exporta todos los registros de control horario.</CardDescription>
                ...
                <Select value={filtroEmpleado} onValueChange={setFiltroEmpleado}>
                 ...
                 <Select value={filtroTipo} onValueChange={setFiltroTipo}>
                    ...
                    <SelectContent>
                        ...
                        {fichajeTypes.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}
                    </SelectContent>
                </Select>
                <Button variant="outline" size="icon" onClick={handleExport} disabled={filteredFichajes.length === 0} aria-label="Exportar a CSV">
            ...
            {filteredFichajes.length > 0 ? (
              filteredFichajes.map((fichaje) => (
                <TableRow key={fichaje.id}>
                  ...
                  <TableCell>{employees.find(e => e.id === fichaje.employeeId)?.name || fichaje.employeeName}</TableCell>
                  <TableCell>
              ...
              <TableRow>
                <TableCell colSpan={5} className="h-24 text-center">
                  No hay registros para este periodo o filtros.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
