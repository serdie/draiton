...
import { Printer, Download, Loader2, Landmark, FileText, Mail, Phone } from 'lucide-react';
...
export function ViewDocumentModal({ isOpen, onClose, document }: ViewDocumentModalProps) {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
        // Si hay un código QR, aseguramos que esté renderizado como imagen
        const qrElements = element.querySelectorAll('.react-qrcode-logo');
        if (qrElements.length > 0) {
            // Esperamos un poco más para asegurar que el QR esté completamente renderizado
        ...
        const canvas = await html2canvas(element, {
            scale: 2,
            useCORS: true,
            backgroundColor: '#ffffff', // Fondo blanco para asegurar contraste
            height: element.scrollHeight,
            windowHeight: element.scrollHeight,
            // Opciones para manejar mejor los SVG
            onclone: (clonedDoc) => {
                // Asegurar que los códigos QR se rendericen como imágenes
                const qrCodes = clonedDoc.querySelectorAll('canvas[data-vqr]');
                if (qrCodes.length > 0) {
                    // Forzar renderizado de QR codes
                    setTimeout(() => {}, 100);
                }
            }
        });
        ...
        <div className="flex-1 overflow-y-auto pr-2 -mr-6 py-4 space-y-6 bg-background">
             <div ref={printableAreaRef} id="printable-area" className="p-8 bg-white text-black text-sm">
                ...
                <div className="text-center mt-16 pt-4 border-t text-xs text-gray-500">
                    ...
                    {/* Información de Verifactu */}
                    {document.verifactuQR && (
                        <div className="mt-6 flex flex-col items-center">
                            <div className="mb-2 flex flex-col items-center">
                                {/* Mostrar el código QR solo si el contenido no es una imagen base64 */}
                                {!document.verifactuQR.startsWith('iVBORw0KGgo') && !document.verifactuQR.startsWith('/9j/') && (
                                    <QRCodeSVG
                                        value={document.verifactuQR}
                                        size={100}
                                        level="L"
                                        includeMargin={true}
                            ...
                            <p className="text-xs text-gray-600 mb-1">Verifica esta factura en la AEAT</p>
                            {document.verifactuChainHash && (
                                <p className="text-xs text-gray-500 mt-1">Huella: {document.verifactuChainHash.substring(0, 30)}...</p>
                            )}
                            {document.verifactuDate && (
                                <p className="text-xs text-gray-500 mt-1">Fecha de registro: {format(new Date(document.verifactuDate), "dd/MM/yyyy HH:mm", { locale: es })}</p>
                            )}
                        </div>
                    )}
                </div>
